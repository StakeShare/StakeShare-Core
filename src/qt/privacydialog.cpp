...
#include "zstakesharecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSSX ought to be enough for anybody." - Bill Gates, 2017
    ui->zSSXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSSXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSSX->setEnabled(false);
        ui->pushButtonMintzSSX->setToolTip(tr("zSSX is currently disabled due to maintenance."));

        ui->pushButtonSpendzSSX->setEnabled(false);
        ui->pushButtonSpendzSSX->setToolTip(tr("zSSX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSSXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSSX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSSX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSSX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSSX in ") +
...
void PrivacyDialog::on_pushButtonSpendzSSX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSSX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSSX
        sendzSSX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSSX
    sendzSSX();
}

void PrivacyDialog::on_pushButtonZStakeShareControl_clicked()
{
    ZStakeShareControlDialog* zSSXControl = new ZStakeShareControlDialog(this);
    zSSXControl->setModel(walletModel);
    zSSXControl->exec();
}

void PrivacyDialog::setZStakeShareControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSSXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSSX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid StakeShare Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSSXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSSX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " STAKESHARE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSSX</b>";
    ...
    // use mints from zSSX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZStakeShareControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZStakeShareControlDialog::GetSelectedMints();
    }
    ...
    // Clear zstakeshare selector in case it was used
    ZStakeShareControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSSX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " StakeShare, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSSX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSSX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSSX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSSX "));
    ui->labelzSSXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSSXSyncStatus->setVisible(fShow);
}
