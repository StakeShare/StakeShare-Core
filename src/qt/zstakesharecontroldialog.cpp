...
#include "ui_zstakesharecontroldialog.h"
...
std::list<std::string> ZStakeShareControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZStakeShareControlDialog::listMints;

ZStakeShareControlDialog::ZStakeShareControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZStakeShareControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZStakeShareControlDialog::~ZStakeShareControlDialog()
{
    delete ui;
}

void ZStakeShareControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZStakeShareControlDialog::updateList()
{
...
void ZStakeShareControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZStakeShareControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZStakeShare_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZStakeShareControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZStakeShareControlDialog::GetSelectedMints()
{
...
void ZStakeShareControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
