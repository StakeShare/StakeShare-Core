...
#if defined(NDEBUG)
#error "StakeShare cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSSX spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zSSX spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zSSX spend with serial %s from tx %s is not in valid range\n",
    ...
    int64_t nSubsidy = 0;
      ...
      if (nHeight == 0) {
             nSubsidy = 440000 * COIN;
                ...
                nSubsidy = 2 * COIN;
      } else if (nHeight <= 63200) {
                nSubsidy = 2 * COIN;
      } else if (nHeight <= 87840) {
                nSubsidy = 2 * COIN;
      } else if (nHeight <= 107840) {
                nSubsidy = 4 * COIN;
      } else if (nHeight <= 131040) {
                nSubsidy = 4 * COIN;
      } else if (nHeight <= 151040) {
                nSubsidy = 5 * COIN;
      } else if (nHeight <= 175680) {
                nSubsidy = 6 * COIN;
      } else if (nHeight <= 195680) {
                nSubsidy = 11 * COIN;
      } else if (nHeight <= 218880) {
                nSubsidy = 11 * COIN;
      } else if (nHeight <= 238880) {
                nSubsidy = 4.9 * COIN;
      } else if (nHeight <= 263520) {
                nSubsidy = 5.15 * COIN;
                ...
                nSubsidy = 8.3 * COIN;
      } else if (nHeight <= 610000) {
                nSubsidy = 7.95 * COIN;
      } else if (nHeight <= 685000) {
                nSubsidy = 7.6 * COIN;
      } else if (nHeight <= 765000) {
                nSubsidy = 7.35 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
      ...
      if (nHeight < 101) {
              ...
              ret = blockValue * 0.8;
      } else if (nHeight < 63201) {
              ret = blockValue * 0.85;
      } else if (nHeight < 87841) {
              ret = blockValue * 0.9;
      } else if (nHeight < 107841) {
              ret = blockValue * 0.85;
      } else if (nHeight < 175681) {
              ret = blockValue * 0.9;
      } else if (nHeight < 195681) {
              ret = blockValue * 0.8;
      } else if (nHeight < 218881) {
              ret = blockValue * 0.85;
      } else if (nHeight < 263521) {
              ret = blockValue * 0.9;
      } else if (nHeight < 310001) {
              ret = blockValue * 0.8;
      } else {
              ret = blockValue * 0.9;
      }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from StakeShare
...
{
    RenameThread("stakeshare-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSTAKESHAREMinted()
{
...
void RecalculateZSTAKESHARESpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
        ...
        //Remove spends from zSSX supply
...
bool RecalculateSTAKESHARESupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zSSX with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSSXSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert STAKESHARE to zSSX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSSX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSSX serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSSX mints and %d zSSX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // StakeShare: We use certain sporks during IBD, so check to see if they are
